    ...
    order() {
        ...
        for (let i = 0, group, n, nd; i < m; ++i) {
            ...
            for (let j = 0, node; j < n; ++j) {
                (node = group[j]) && (nd = node) && ((node as any).__index__ = j);
            }
            nd && nd.parent?.children.sort((a, b) => (a as any).__index__ - (b as any).__index__);
            for (let j = 0, node; j < n; ++j) {
                (node = group[j]) && (delete (node as any).__index__);
            }
        }
        return this;
    }
